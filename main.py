...
# 训练模型
def train_model(opts, cnn, train_dataloader, val_dataloader, model_name):
    criterion = nn.CrossEntropyLoss()                     #交叉熵损失                 
    optimizer = optim.SGD(cnn.parameters(), lr=learning_rate, momentum=0.9, weight_decay=5e-4)
#     optimizer = optim.Adam(cnn.parameters(), lr=learning_rate)
    # if model_name == 'resnet':
    #     output_params = list(map(id, cnn.model.fc.parameters()))
    #     feature_params = filter(lambda p: id(p) not in output_params, cnn.model.parameters())
    #     optimizer = optim.Adam([{'params': feature_params, 'lr': learning_rate * 0.1},
    #                             {'params': cnn.model.fc.parameters()}], 
    #                            lr=learning_rate)
    ...
    for epoch in range(n_epochs):
        cnn.train()                                           ###启用 BatchNormalization 和 Dropout
        num=0
        accu=0
        Loss=0
    ...
    #         print(pred_y, y_label)
        ...
        cnn.eval()  
        train_acc = accu / num
        # train_sensitivity = accu1 / num1
        # train_specificity = accu2 / num2
        train_loss = Loss / num
#         if epoch % 10 == 0:
        torch.save(cnn.state_dict(),f'checkpoints_model/{model_name}/{opts.model}_parameter.pkl') 
    
        # train_acc, train_loss, train_sensitivity, train_specificity = test_model(cnn, train_dataloader)
        val_acc, val_loss = test_model(cnn, val_dataloader)
        
        if best_val_acc <= val_acc:
            best_val_acc = val_acc
            torch.save(cnn.state_dict(),f'checkpoints_model/{model_name}/{opts.model}_best_parameter.pkl') 
        ...
        fp = open(f'draw_result/{model_name}/{opts.model}_acc.csv','a+')
        acc_result = []
        ...
        # fp = open(f'draw_result/{model_name}/{opts.model}_sensitivity.csv','a+')
        # sensitivity_result = []
        # sensitivity_result.append(epoch)
        # sensitivity_result.append(train_sensitivity)
        # sensitivity_result.append(val_sensitivity)
        # context = csv.writer(fp,dialect='excel')       # 定义一个变量进行写入，将刚才的文件变量传进来，dialect就是定义一下文件的类型，我们定义为excel类型
        # context.writerow(sensitivity_result)
        # fp.close()
        
        # fp = open(f'draw_result/{model_name}/{opts.model}_specificity.csv','a+')
        # specificity_result = []
        # specificity_result.append(epoch)
        # specificity_result.append(train_specificity)
        # specificity_result.append(val_specificity)
        # context = csv.writer(fp,dialect='excel')       # 定义一个变量进行写入，将刚才的文件变量传进来，dialect就是定义一下文件的类型，我们定义为excel类型
        # context.writerow(specificity_result)
        # fp.close()
        
        
        fp = open(f'draw_result/{model_name}/{opts.model}_loss.csv','a+')
        loss_result = []
    ...
    # plt.figure()
    # plt.plot(range(n_epochs),Train_accuracy,"r", label="train accuracy")
    # plt.plot(range(n_epochs),Val_accuracy,"g", label="val accuracy")
    
    # plt.ylabel('Accurary')
    # plt.xlabel('epoch')
    # plt.title('Accurary---epoch')
    # plt.legend(loc='upper left')
    
    # plt.savefig(f'draw_result/{opts.model}_result.jpg')
    return
