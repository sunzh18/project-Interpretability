...
def trade_train_model(opts, cnn, train_dataloader, val_dataloader, model_name):
    ...
    for epoch in range(n_epochs):
        ...
        for data, target in tqdm(train_dataloader):       
            ...
            data, target = data.to(device), target.to(device)    
            with autocast():
                loss, Natural_output, Robust_output = trades_loss(model=cnn,
                           x_natural=data,
                           y=target,
                           optimizer=optimizer,
                           beta=opts.beta)                                    
            ...
            Natural_pred_y = torch.max(Natural_output, 1)[1].cpu().numpy()
            Robust_pred_y = torch.max(Robust_output, 1)[1].cpu().numpy()
            y_label = target.cpu().numpy()
            Natural_accu += (Natural_pred_y == y_label).sum()
            Robust_accu += (Robust_pred_y == y_label).sum()
    #         print(pred_y, y_label)
        ...
        scheduler.step()
        cnn.eval()  
        train_Natural_acc = Natural_accu / num
...
#         if epoch % 10 == 0:
        ...
        print("epoch =",epoch,":\ntrain-- Natural_accuracy =",train_Natural_acc,":\ntrain-- Robust_accuracy =",train_Robust_acc,"loss =",train_loss)
...
if __name__ == '__main__':
    ...
    trade_train_model(opts, cnn, train_dataloader, val_dataloader, model_name)
